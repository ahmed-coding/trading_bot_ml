...
max_open_trades = 8  # الحد الأقصى للصفقات المفتوحة في نفس الوقت (تم تقليله لتقليل المخاطر)
...
def monitor_trades():
    ...
    for symbol, trade in list(active_trades.items()):
        try:
            current_price = float(client.get_symbol_ticker(symbol=symbol)['price'])
            result = None
            # remaining_time = (trade['timeout'] - (time.time() - trade['start_time'])) / 60
            # print(f"{datetime.now()} - مراقبة الصفقة لـ {symbol}: السعر الحالي {current_price}, الوقت المتبقي {remaining_time:.2f} دقيقة")
