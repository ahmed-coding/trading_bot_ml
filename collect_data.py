...
def collect_data(symbol, interval='3m', limit=100):
    ...
    with open(f"data/{symbol}_scalping_data.csv", mode="w", newline="") as file:
        ...
        for i in range(15, len(klines)):
            ...
            gains = np.mean([diff for diff in np.diff(close_prices) if diff > 0])
            losses = np.mean([-diff for diff in np.diff(close_prices) if diff < 0])
...
if __name__ == "__main__":
    symbols = get_top_symbols(limit=20, profit_target=0.003)  # Automatically fetch top symbols
